...
tmux send-keys "cd ~/htb/endgame/ && mkdir $BOX.htb; cd $BOX.htb; mkdir www; mkdir nmap" C-m
...
tmux selectp -t 1 && tmux send-keys "openvpn ~/htb/tools/JaguarZz-endgame.ovpn"  C-m && tmux split-window -v

# Selects pane 2, runs cherrytree
tmux selectp -t 2 && sleep .1 && tmux send-keys "cherrytree ~/htb/endgame/$BOX.htb/$BOX.ctb" C-m
...
; nmap -sC -sV -T4 -p- -oN ~/htb/endgame/$BOX.htb/nmap/full_scan.nmap  $IP" C-m # nmap
